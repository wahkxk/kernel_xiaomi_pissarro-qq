...
static void print_send_buffer(struct pn544_dev *nqx_dev, unsigned char *buf, int len)
{
    ...
    for (i = 0; i < len; i++) {
        snprintf(output + i * 2, 4, "%02x ", buf[i]);
    }
...
long p61_cold_reset(void)
{
    ...
    do {
        ret = i2c_master_send(g_pn544_dev->client, cmd_ese_cold_reset, sizeof(cmd_ese_cold_reset));
    ...
    case PN544_SET_PWR:
        {
            ...
            dev_warn(&pn544_dev->client->dev, "set power  arg: %ld  current state:0x%x\n", arg, current_state);
            if (arg == 2) {
#if 0
                if (current_state &
                ...
                if ((pn544_dev->spi_ven_enabled == false
                    ...
                    dev_warn(&pn544_dev->client->dev, "set ven HIGHT\n");
                    gpio_set_value(pn544_dev->ven_gpio, 1);
                    msleep(10);
                    if (pn544_dev->firm_gpio) {
                        p61_update_access_state(pn544_dev, P61_STATE_DWNLD, true);
                        dev_warn(&pn544_dev->client->dev, "set firmware HIGHT\n");
                        gpio_set_value(pn544_dev->firm_gpio, 1);
#if 0
                    }
#endif
                    msleep(10);
                    dev_warn(&pn544_dev->client->dev, "set ven LOW\n");
                ...
                if (pn544_dev->firm_gpio) {
                    ...
                    dev_warn(&pn544_dev->client->dev, "set firmware LOW\n");
    ...
    case P61_SET_SPI_PWR:
        {
            ...
            if (arg == 1) {
                dev_warn(&pn544_dev->client->dev, "%s : PN61_SET_SPI_PWR - power on ese\n", __func__);
                if ((current_state & (P61_STATE_SPI | P61_STATE_SPI_PRIO)) == 0) {
                    ...
                    if (!(current_state & P61_STATE_JCP_DWNLD)) {
                        if (pn544_dev->nfc_service_pid) {
                            ...
                            pr_info("invalid nfc service pid");
                        }
                    }
...
//#endif
                ...
                dev_warn(&pn544_dev->client->dev,"%s : PN61_SET_SPI_PWR - power off ese\n",__func__);
                if (current_state & P61_STATE_SPI_PRIO) {
                    p61_update_access_state(pn544_dev, P61_STATE_SPI_PRIO, false);
                    if (! (current_state & P61_STATE_JCP_DWNLD)) {
                        if (pn544_dev->nfc_service_pid) {
                            if (!(current_state &P61_STATE_WIRED)) {
                                svdd_sync_onoff (pn544_dev->nfc_service_pid, P61_STATE_SPI_SVDD_SYNC_START | P61_STATE_SPI_PRIO_END);
                            } else {
               ...
               const struct i2c_device_id *id)
{
    ...
    if (client->dev.of_node) {
        platform_data = devm_kzalloc(&client->dev, sizeof(struct pn544_i2c_platform_data), GFP_KERNEL);
        if (!platform_data) {
    ...
    g_pn544_dev->pSecureTimerCbWq = create_workqueue(SECURE_TIMER_WORK_QUEUE);
    ...
    ret = request_irq(client->irq, pn544_dev_irq_handler, IRQF_TRIGGER_HIGH, client->name, g_pn544_dev);
    ...
    dev_warn(&client->dev, "set ven LOW\n");
    gpio_set_value(g_pn544_dev->ven_gpio, 0);
    msleep(5);
    dev_warn(&client->dev, "set ven HIGHT\n");
    ...
    ret1 = pmic_config_interface((MT6359_DCXO_CW19), 3, (PMIC_RG_XO_EXTBUF3_RSEL_MASK), (PMIC_RG_XO_EXTBUF3_RSEL_SHIFT));
